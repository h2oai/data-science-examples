...
.locate.path.compute<-
  function(cur.dir, root, root.parent = NULL) {
    ...
    if (is.null(root.parent)) {
      ...
      # the root is the parent
      if (parent.name == root && basename(dirname(parent.dir)) == root.parent) return(.locate.path.compute(parent.dir, root, root.parent)) #return(normalizePath(paste(parent.dir, "/", root, sep = "")))

    }
    return(.locate.path.compute(parent.dir, root, root.parent))
  }
